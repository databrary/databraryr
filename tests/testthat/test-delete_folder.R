...
test_that("delete_folder deletes an existing folder", {
  created <- create_folder(vol_id = TEST_VOL_ID, name = "delete_folder happy path", vb = FALSE)
  skip_if_null_response(created, "create_folder for delete_folder happy path")

  folder_id <- created$id
  on.exit(
    try(delete_folder(vol_id = TEST_VOL_ID, folder_id = folder_id, vb = FALSE), silent = TRUE),
    add = TRUE
  )

  result <- delete_folder(vol_id = TEST_VOL_ID, folder_id = folder_id, vb = FALSE)

  expect_true(result)

  # Verify it's gone
  expect_null(get_folder_by_id(folder_id = folder_id, vol_id = TEST_VOL_ID, vb = FALSE))
})
...
test_that("delete_folder works with verbose mode", {
  created <- create_folder(vol_id = TEST_VOL_ID, name = "delete_folder vb", vb = FALSE)
  skip_if_null_response(created, "create_folder for delete_folder vb")

  on.exit(
    try(delete_folder(vol_id = TEST_VOL_ID, folder_id = created$id, vb = FALSE), silent = TRUE),
    add = TRUE
  )

  expect_true(delete_folder(vol_id = TEST_VOL_ID, folder_id = created$id, vb = TRUE))
})
