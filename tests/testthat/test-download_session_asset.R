...
test_that("download_session_asset rejects bad input parameters", {
  expect_error(download_session_asset(vol_id = -1))
  expect_error(download_session_asset(vol_id = 0))
  expect_error(download_session_asset(vol_id = "a"))
  expect_error(download_session_asset(vol_id = list(a = 1, b = 2)))
  expect_error(download_session_asset(vol_id = TRUE))
  ...
  expect_error(download_session_asset(timeout_secs = -1))
  expect_error(download_session_asset(timeout_secs = 0))
  expect_error(download_session_asset(timeout_secs = list(1, 2)))
