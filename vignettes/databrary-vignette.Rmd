...
First, we need to download and install the `devtools` package and then the `databraryapi` package from GitHub.
...
this_session <- list_sessions()[22,'session_id']
this_vol <- list_sessions()[22,'vol_id']
list_assets_in_session(session_id = this_session, vol_id = this_vol)
```
...
Clearly, there are several files here, `r dim(list_assets_in_session(session_id = this_session, vol_id = this_vol))[1]` in fact.
And all are videos.
How do I know that?
...
list_assets_in_session(vol_id = m$vol_id, session_id = m$session_id)
```
...
df %>%
  ...
  geom_line(size = 3) +
  ylab("Authorizations") +
  scale_y_continuous(breaks = seq(0, 1100, 100), expand = c(0,0)) +
  theme_bw() +
