...
```{r}
vol1_df <- databraryr::list_volume_assets()
```
...
vol1_df_new <- dplyr::left_join(vol1_df, these_formats_df, by=dplyr::join_by(asset_format_id==id))
...
```{r}
databraryr::list_volume_metadata()
```

This command can also be "vectorized."

```{r}
purrr::map(c(1:50), databraryr::list_volume_metadata) |>
  purrr::list_rbind()
```
...
```{r}
purrr::map(c(1:20), databraryr::list_volume_funding) |>
  purrr::list_rbind()
```
