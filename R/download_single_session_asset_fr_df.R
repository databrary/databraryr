...
#'
#' @description
#' Helper used by `download_session_assets_fr_df()` to fetch a single asset via
#' the signed-download workflow.
#'
#' @param i Integer. Index of the asset within `session_df`.
#' @param session_df Data frame containing asset metadata.
...
download_single_session_asset_fr_df <- function(i = NULL,
  ...
  this_asset <- session_df[i, , drop = FALSE]
  ...
  dest_dir <- if (isTRUE(add_session_subdir)) {
    file.path(target_dir, this_asset$session_id)
  } else {
    target_dir
  }
  ...
  download_session_asset(
    ...
    session_id = this_asset$session_id,
