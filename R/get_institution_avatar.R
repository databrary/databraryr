...
get_institution_avatar <- function(institution_id = 1,
  ...
  assertthat::assert_that(institution_id == floor(institution_id),
                          msg = "institution_id must be an integer")
  ...
  tryCatch({
    ...
    if (status != 200) {
      if (vb) {
        message("Institution ", institution_id, " avatar not found or inaccessible (status: ", status, ")")
      }
      return(NULL)
    }
    ...
    if (is.null(dest_path)) {
      ...
      if (vb) {
        message("Saved avatar to: ", final_path, " (", length(avatar_bytes), " bytes)")
      }
    ...
    if (vb) {
      message("Error downloading avatar for institution ", institution_id, ": ", e$message)
    }
    return(NULL)
  })
}