...
list_affiliates <- function(party_id = 6, report_target_party = FALSE,
  ...
  list_single_party_affiliates <- function(party_id = NULL, report_target_party = NULL,
                                    vb = NULL) {
    ...
    if (!is.null(g)) {
      ...
      if (!is.null(p)) {
        p |>
          ...
          dplyr::rename(affiliate_id = id,
                        last_name = sortname,
                        first_name = prename) |>
          dplyr::select(party_id, affiliate_id, last_name, first_name, affiliation, url)
      } else {
        NULL
      }
    } else {
  ...
  #------------------------------------------------------------
  if (vb) message("Summarizing affiliates for n=", length(party_id), " parties.")
  purrr::map(party_id,
