...
list_session_assets <-
  function(vol_id = 1,
    ...
    # Select session info
    these_sessions <- purrr::map(vol_list$containers, get_sessions) |>
      purrr::list_rbind()
    
    session_match <- (session_id == these_sessions$session_id)
    if (sum(session_match) == 0) {
      if (vb) message("No matching session_id: ", session_id)
      return(NULL)
    }
    session_match_index <- seq_along(session_match)[session_match]
    
    this_session <- vol_list$containers[[session_match_index]]
    if (is.null(this_session)) return(NULL)
    
    assets_df <-
      purrr::map(vol_list$containers[[session_match_index]]$assets, as.data.frame) |>
      purrr::list_rbind()
    ...
    dplyr::left_join(assets_df, asset_formats_df, by = dplyr::join_by(asset_format_id == format_id))
  }

#-------------------------------------------------------------------------------
#' Helper function for list_session_assets
get_sessions <- function(volume_container) {
  session_df <- tibble::tibble(session_id = volume_container$id,
                               session_name =
                                 volume_container$name)
